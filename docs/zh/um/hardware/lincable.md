...
- **自定义脉冲序列**：在 LIN 总线上发送任意高/低脉冲序列（LIN 空闲高电平，交替高/低）
- **位时间单位**：每个元素以当前波特率为基准的位时间单位
- **数组长度即数量**：数组长度 = 脉冲段数（最多 64）
- **仅脉冲**：设置后仅发送自定义脉冲（不发送 LIN 帧）
- **使用场景**：LIN 唤醒模式、Break 时序测试、自定义信号注入

通过 `frame.lincable.customPulses` 配置（长度为数组）：

```typescript
import { output, LinDirection, LinChecksumType } from 'ECB'

// 发送自定义脉冲：高 10 位时间，低 5 位时间
const wakeupFrame = {
  frameId: 0,
  data: Buffer.alloc(0),
