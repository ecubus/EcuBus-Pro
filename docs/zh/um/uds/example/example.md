...
本指南演示如何使用 EcuBus-Pro 和 S32K144 微控制器实现完整的 UDS（统一诊断服务）引导加载程序。 该示例涵盖了从初始配置到脚本实现的整个固件更新过程。
...
![寻址模式配置](images/03-addressing-mode-config.png)
...
![硬件复位配置](images/06-hardware-reset-config.png)

#### SecurityAccess（安全访问）($27) 服务

- 种子请求配置如下。 您需要进入 Response 接口并将 securitySeed 长度更改为 128 位，否则无法接收到 MCU 回复的完整种子数据。
...
- 例程 ID 为 0x0202 的例程用于通知 MCU 执行 CRC 校验，配置如下：

![例程控制 CRC 配置](images/11-routine-control-crc-config.png)

- 例程 ID 为 0xFF00 的例程用于通知 MCU 执行 Flash 擦除，配置如下。

![Flash 擦除配置](images/12-routine-control-flash-erase-config.png)

- 例程 ID 为 0xFF01 的例程用于通知 MCU 执行固件验证，配置如下：

![固件验证配置](images/13-routine-control-firmware-check-config.png)

#### RequestDownload（请求下载）($34) 服务

- 请求下载配置如下。 存储地址和存储空间将在实际使用中在脚本中分配。

![请求下载配置](images/14-request-download-config.png)

#### TransferData（传输数据）($36) 服务

- 传输数据配置如下。 在实际使用中，它将在脚本中被重复调用并赋值。
...
![请求传输退出配置](images/16-request-transfer-exit-config.png)

#### JobFunction

> 什么是 Job？ Job 是 EcuBus-Pro 中的一个抽象服务。 使用 Job 时，必须有相应的脚本。 通过脚本，实现 Job 的返回。 Job 的返回要求是一个数组，可以是 0-N 个普通 UDS 服务或 Jobs。 Job 通常用于固件下载、上传以及其他需要多个未知数量的 0x36 服务的场景，但也可以用于您想要的任何其他情况。
...
4. 通过 $34 服务发起闪存驱动程序下载请求；
5. 通过 $36 服务传输闪存驱动程序；
6. 通过 $37 服务结束传输；
7. 通过 $31 服务通知 MCU 执行 CRC 校验和 Flash 擦除；
8. 通过 $34 服务发起 APP 下载请求；
9. 通过 $36 服务传输 APP；
10. 通过 $37 服务结束传输；
11. 通过 $31 服务通知执行 CRC 校验和固件检查；
...
![创建新的 TS 文件](images/19-create-ts-file.png)

- 将升级所需的文件放置在项目目录中；

![将文件放置在项目目录中](images/20-place-upgrade-files.png)

- 在 UDS Tester 中加载脚本文件并打开 VS Code 进行脚本编写；

![在 UDS Tester 中加载脚本文件](images/21-load-script-vscode.png)
...
_此示例可作为在汽车应用中实现 UDS 引导加载程序的基础。_ _请根据您的具体项目需求自定义配置和脚本。_
