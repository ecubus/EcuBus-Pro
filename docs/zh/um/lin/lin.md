...
LIN是一种经济高效且确定性的通信协议，专为连接电子控制单元（ECU）与智能传感器、执行器和控制器而设计。 EcuBus-Pro的LIN模块提供全面的功能，用于根据LIN 2.x规范开发、分析和测试LIN网络。
...
3. **信号值编辑器**
   ...
   - 修改信号值（**仅可修改已配置节点发布的信号。 例如，在下图中，仅可修改`Motor2`作为发布者的信号值**）
