...
LIN是一种经济高效且确定性的通信协议，专为连接电子控制单元（ECU）与智能传感器、执行器和控制器而设计。 LIN是一种经济高效且确定性的通信协议，专为连接电子控制单元（ECU）与智能传感器、执行器和控制器而设计。 EcuBus-Pro的LIN模块提供全面的功能，用于根据LIN 2.x规范开发、分析和测试LIN网络。

## 添加设备

通过顶部菜单栏，选择`Hardware`，然后点击支持LIN的设备右侧的`+`按钮以添加设备。
...
> [!IMPORTANT]
> 下面描述的一些功能可能需要导入LIN描述文件（LDF）。 有关LDF的更多信息，请参阅[数据库文档](./../ldf)。 有关LDF的更多信息，请参阅[数据库文档](./../ldf)。
...
调度表管理允许定期执行LDF数据库中的特定调度表。

### 添加LIN交互

- 打开网络界面
- 点击网络左侧`Lin`网络下`Interaction`选项卡下的`+`按钮以添加交互
...
- 查看和管理现有调度表。 您可以通过关闭Active来临时跳过调度表中的一帧。
...
配置和仿真LIN节点以测试网络行为，并动态更新相应节点的信号值。

### 节点配置步骤

1. **添加节点**
   - 打开网络界面
   - 点击网络左侧`Node`选项卡下的`+`按钮以添加节点
...
3. **信号值编辑器**
   ...
   - 修改信号值（\*\*仅可修改已配置节点发布的信号。 修改信号值（**仅可修改已配置节点发布的信号。 例如，在下图中，仅可修改`Motor2`作为发布者的信号值**）
     ![Signal Value Editor](../../../media/um/lin/image-4.png)

4. **节点仿真**
   节点配置网络节点后，将自动仿真节点发送信号。
   例如，当设备Lin工作在主节点模式且Node1配置为`Motor2`从节点网络节点时，当收到`Motor2`的请求时，Node1将自动响应此帧并给出响应。
   例如，当设备Lin工作在主节点模式且Node1配置为`Motor2`从节点网络节点时，当收到`Motor2`的请求时，Node1将自动响应此帧并给出响应。
