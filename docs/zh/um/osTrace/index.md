...
OSEK OS 追踪是一项强大的功能，允许您通过外部接口实时获取 OSEK OS 的运行时状态，并将其转换为可视化的图形界面。
...
通过点击 `其他->数据库->添加 OS(ORTI)` 导入 ORTI 文件。 您还可以查看已导入的 ORTI 文件。
...
1. 设置名称
2. 设置实际的 CPU 工作频率（取决于您的时间戳格式）。 如果您的时间戳已经是微秒单位，CPU 频率可以设为 1。
3. 设置对象颜色
...
记录文件决定了追踪数据写入本地文件的位置。 写入的数据格式为 CSV 格式。
...
可通过`Others->Os Info->[对应ORTI文件名] Statistics`查看OS统计信息。
...
可通过`Others->Os Info->[对应ORTI文件名] Timeline`查看OS时间线。

时间线使用`pixi.js`绘制，并通过GPU加速。
...
您可以打开先前保存的CSV记录文件，在离线模式下查看OS时间线。
![offline](./../../../media/um/osTrace/offline.png)

> [!NOTE]
> 如果启用了`Link Trace`并且跟踪窗口已打开，离线事件也会附加到跟踪窗口中。
