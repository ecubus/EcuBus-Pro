<template>
  <el-container
    class="_fc-designer"
    :style="height ? `height:${dragHeight};flex:0;` : ''"
    ...
    <el-main>
      <el-container style="height: 100%" :key="locale && locale.name">
          ...
          <el-container style="height: 100%" v-if="activeModule === 'base'">
            ...
            <el-main v-show="activeMenuTab === 'menu'">
              <template v-for="(item, index) in menuList">
                <div class="_fc-l-group" :key="index" v-if="hiddenMenu.indexOf(item.name) === -1">
                  ...
                  <div class="_fc-tree-node" :class="{ active: activeRule === data.rule }">
                    <div class="_fc-tree-label">
                      ...
                      <span>{{
                        ...
                        t('com.' + (data.rule._menu && data.rule._menu.name) + '.name') ||
                        (data.rule._menu && data.rule._menu.label) ||
                        data.rule.type
                      }}</span>
                    </div>
          ...
          <el-header class="_fc-m-tools" height="45">
            <div class="_fc-m-tools-l">
              <template v-if="!inputForm.state">
                <template v-if="getConfig('showDevice') !== false">
                  <div class="devices">
                    <i
                      class="fc-icon icon-pc"
                      :class="{ active: device === 'pc' }"
                      @click="setDevice('pc')"
                    ></i>
                    <i
                      class="fc-icon icon-pad"
                      :class="{ active: device === 'pad' }"
                      @click="setDevice('pad')"
                    ></i>
                    <i
                      class="fc-icon icon-mobile"
            ...
            <div class="_fc-m-tools-r">
              <template v-if="!inputForm.state">
                ...
                <el-button type="primary" plain size="small" @click="openPreview"
                  ><i class="fc-icon icon-preview"></i> {{ t('props.preview') }}
                </el-button>
                <el-button type="danger" plain size="small" @click="clearDragRule"
          ...
          <el-container style="height: 100%">
            <el-header height="40px" class="_fc-r-tabs">
              <div
                ...
                v-if="
                  !!activeRule || customForm.isShow || (config && config.showFormConfig === false)
                "
            ...
            >
              ...
              <template
                v-if="
                  activeRule ||
                  (customForm.config && (customForm.config.name || customForm.config.label))
                "
              >
              ...
              <div class="_fc-r-config" :style="{ 'grid-template-areas': configFormOrderStyle }">
                ...
                <div v-if="activeRule" style="grid-area: validate">
                  <el-divider v-if="validateForm.isShow">{{ t('designer.validate') }} </el-divider>
                  <DragForm
        ...
        <el-dialog
          ...
          align-center
          :append-to="`#win${getConfig('editIndex')}`"
          :style="{
        ...
        >
          ...
          ><code v-html="preview.component"></code></pre>
          <pre
            class="_fd-preview-code"
            ref="previewCode"
            v-else
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const methods = {
      ...
      clearDragRule() {
        ElMessageBox.confirm(t('designer.clearWarn'), '', {
          confirmButtonText: t('props.clear'),
          cancelButtonText: t('props.cancel'),
          type: 'warning',
          buttonSize: 'small',
          appendTo: `#win${methods.getConfig('editIndex')}`
        }).then(() => {
          methods.setRule([])
      ...
      setComponentRuleConfig(name, rule, append) {
        ...
        if (activeRule) {
          ...
          data.propsForm.rule = data.cacheProps[activeRule._fc_id] = tidyRuleConfig(
            activeRule._menu.props,
            componentRule.value && componentRule.value[activeRule._menu.name],
            activeRule,
            {
              t,
      ...
      setOption(opt) {
        ...
        options._resetBtn =
          typeof options.resetBtn === 'object'
            ? options.resetBtn
            : { show: options.resetBtn === true }
        options._submitBtn =
          typeof options.submitBtn === 'object'
            ? options.submitBtn
            : { show: options.submitBtn !== false }
      ...
      mergeOptions(options) {
        ...
        if (
          options.style &&
          (!data.formOptions.style || data.formOptions.style.indexOf(options.style) === -1)
        ) {
      ...
      validateChange(field, value, _, fapi) {
        if (!data.activeRule || data.validateForm.api[data.activeRule._fc_id] !== data.activeRule)
          return
        methods.handleChange('', field, value, _, fapi)
      ...
      getPropsRule(rule) {
        let propsRule = tidyRuleConfig(
          rule._menu.props,
          componentRule.value && componentRule.value[rule._menu.name],
          rule,
          {
            t,
            api: data.dragForm.api
      ...
      toolActive(rule) {
        ...
        data.baseForm.isShow =
          input && rule.input !== false && methods.getConfig('showBaseForm') !== false
        data.propsForm.isShow =
          data.cacheProps[rule._fc_id].length > 0 && methods.getConfig('showPropsForm') !== false
        data.eventShow =
          rule._menu.event &&
          rule._menu.event.length > 0 &&
          methods.getConfig('showEventForm') !== false
        data.styleForm.isShow =
          rule._menu.style !== false && methods.getConfig('showStyleForm') !== false
        const showValidateForm = methods.getConfig('showValidateForm')
        data.validateForm.isShow =
          ((data.baseForm.isShow && showValidateForm !== false) || showValidateForm === true) &&
      ...
      getConfig(key, def) {
        return configRef.value
          ? hasProperty(configRef.value, key)
            ? configRef.value[key]
            : def
          : def
      },
      ...
      checkOnly(menu) {
        ...
        data.dragForm.api.all().forEach((rule) => {
          flag =
            flag || rule._fc_template === menu.name || (rule._menu && rule._menu.name === menu.name)
        })
        if (flag) {
          errorMessage(
      ...
      getSlotConfig(pConfig, slot, config) {
        ...
        return {
          ...config,
          dragBtn: false,
          handleBtn: config.children ? ['addChild'] : false,
          ...slotConfig
        }
      },
        ...
        if (config.drag) {
          rule.children = [
            (drag = methods.makeDrag(
              config.drag,
              rule._menu ? rule._menu.name : rule.type,
              children,
              {
        ...
        if (config.inside) {
          ...
          return {
            ...
            on: {
              delete: ({ self }) => {
                if (
                  methods.handleRemoveBefore({ parent: self, rule: self.children[0] }) !== false
                ) {
      ...
      bindHotkey(event) {
        ...
        if (
          ...
          ['ArrowUp', 'ArrowDown', 'c'].indexOf(event.key) > -1 &&
          data.activeRule
        ) {
