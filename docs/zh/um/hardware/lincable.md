...
- **自定义脉冲序列**：在 LIN 总线上发送任意的高/低脉冲序列（LIN 空闲时为高电平，交替高/低电平）
- **位时间单位**：每个元素的长度基于当前波特率以位时间单位表示
- **数组长度 = 计数**：数组长度 = 脉冲计数（最多 64 个）
- **仅脉冲**：设置后，仅发送自定义脉冲（无 LIN 帧）
- **使用场景**：LIN 唤醒模式、中断时序测试、自定义信号注入

通过 `frame.lincable.customPulses`（长度数组）配置：
