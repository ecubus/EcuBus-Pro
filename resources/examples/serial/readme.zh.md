...
与 `script_serial_port` 示例（在脚本里用 `SerialPortClient` 按路径直接打开端口）不同，这里串口是一个被配置的设备：
...
- 这种「设备绑定」方式是集成串口硬件的推荐做法；仅在不需要设备配置和 Trace 的临时按路径访问场景，才使用独立的 `SerialPortClient`（见 `script_serial_port` 示例）。
