...
由于 EcuBus-Pro 脚本基于 Node.js，您可以在脚本中使用任何 Node.js 包。 有两种安装包的方法：

## 方法 1：使用包管理器界面

首次使用项目时，您需要在安装包之前初始化 package.json 文件。

EcuBus-Pro 提供了一个图形化包管理器界面，便于安装、管理和卸载包。

1. 对于新项目，您会看到消息 "未找到 package.json" 和一个 "初始化 package.json" 按钮。 点击此按钮以创建新的 package.json 文件。
...
您还可以在 EcuBus-Pro CLI 中使用 `pnpm` 命令安装包。 有关更多详细信息，请参阅 [EcuBus-Pro CLI](cli.md)。
