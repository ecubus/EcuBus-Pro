...
用户可以根据配置的 UDS 服务生成 C 代码。 代码格式取决于用户定义的模板。
![code](./images/uds-code-format.png)

## UDS 代码生成

首先，用户需要在 UDS 测试器中启用 UDS 代码生成。
...
支持多个模板配置。 用户可以根据需要添加或删除模板配置，每个模板支持运行时预览功能。

## 特殊属性

启用 UDS 代码生成时，每个服务支持为用户定义的 `代码生成` 添加特殊属性。
...
UDS 代码生成系统基于 [**Handlebars.js**](https://handlebarsjs.com) 模板引擎，通过大量自定义辅助方法增强，提供强大的代码生成能力。

### 模板系统概述

<span v-pre>
模板使用 Handlebars 语法，双花括号 `{{ }}` 用于表达式，`{{# }}{{/ }}` 用于块辅助函数。 系统提供对 UDS 服务数据、配置属性以及丰富辅助函数集的访问。
</span>
...
- <span v-pre>`{{trim str}}`</span> - 去除空白字符
