# 详细EcuBus-Pro开发环境设置指南

本指南提供设置EcuBus开发环境的详细说明。 开始之前，请准备以下内容：

1. Node.js - 从官方网站下载最新版本
2. EcuBus源代码 - 从GitHub下载最新版本
...
确保勾选上图中突出显示的复选框。 这将自动安装Node.js所需的Python和VS2019。 即使您已经安装了这些程序，我们也建议勾选此复选框。
...
下一个屏幕将检查您的计算机是否已安装Python和VS。 如果没有，它将安装它们。 首次安装可能需要一些时间：

![11](../../media/dev/detail/11.jpg)

由于我的计算机已经安装了Python 3.13.3和VS2019，这些步骤被跳过了：

![12](../../media/dev/detail/12.jpg)

Python 3.13.3和VS2019开发环境现已安装。 您可以退出此屏幕。

## 步骤2：安装NPM包

我们已经完成了Node.js的安装。 现在让我们安装NPM包。

导航到EcuBus项目目录（在此示例中为E:\EcuBus\EcuBus_install\EcuBus-Pro-master）：
...
如图所示，有104个错误。 这个问题困扰了我很长时间。 起初，我认为是环境问题，但**实际上是由于.h文件的编码格式**。

![17](../../media/dev/detail/17.jpg)

用记事本打开toomoss文件夹，我们发现offline_type.h采用UTF-8编码：

![18](../../media/dev/detail/18.jpg)

将格式更改为带BOM的UTF-8并重新编译：

![19](../../media/dev/detail/19.jpg)

现在错误减少到75个，tsfn.cxx文件存在问题：

![20](../../media/dev/detail/20.jpg)

将tsfn.cxx文件编码更改为BOM格式：
...
首先，我们修改tsfn.cxx文件的编码格式：

![26](../../media/dev/detail/26.jpg)

编译仍然失败，出现相同的错误。 为了识别哪个文件有格式问题，在VS2019或VS2022中打开项目：

![27](../../media/dev/detail/27.jpg)

在VS2019中编译时，我们得到以下错误：

![28](../../media/dev/detail/28.jpg)

我们发现usb2lin_ex.h文件存在编码问题。 修改后重新编译：
